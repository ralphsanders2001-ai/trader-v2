...
DB_PATH           = f"{DATA_DIR}/trader_live.db"  # 2026-08-10: separate paper/live DBs
...
SCAN_INTERVAL        = 300   # 5 min — full watchlist scan (was 600/10min)
...
ENABLE_NO_TRADE_WINDOW = False  # FIX 2026-08-06: Disabled per user request — no post-open delay
...
LOSS_CAP_PCT         = 0.15      # 15% of position value (FIX 2026-08-05: was 12% — too tight with bid-ask spread)
LOSS_CAP_DOLLAR      = 40.00     # $40 max loss per trade (FIX 2026-08-05: was $25 — accounts for 10-15% bid-ask spread)
...
MAX_PROFIT_DOLLAR    = 50.00   # Max profit cap raised from $30 — expensive options need room
TAKE_PROFIT_DOLLAR   = 10.00   # Sell at +$10 profit — covers both:
...
LOSS_CAP_DOLLAR_V2   = 30.00   # FIX 2026-08-13: Raised from $15 to $30
...
# POSITION LIMITS
# =============================================================================
MAX_HOLD_MINUTES = 60   # FIX 2026-08-10: bumped from 38 — bot kept killing winners before they hit profit target
...
POSITION_SIZE_CONTRACTS = 1     # default contracts per trade

# =============================================================================
# COOLDOWN (progressive per symbol)
# =============================================================================
COOLDOWN_FIRST_LOSS_MINUTES  = 30   # after 1st loss
COOLDOWN_SECOND_LOSS_MINUTES = 60   # after 2nd loss
COOLDOWN_THIRD_LOSS_ACTION   = "BLOCK"  # after 3rd loss, block rest of day

# =============================================================================
# CIRCUIT BREAKER (daily)
# =============================================================================
MAX_DAILY_LOSS       = 250.0    # FIX 2026-08-07: lowered from $500 to $250 per user — will scale up to $500 when account balance > $2,000, $750 when > $3,000
...
# =============================================================================
USE_ML_GATE          = True     # Use trained XGBoost model to gate entries
ML_SKIP_THRESHOLD    = 0.55     # FIX 2026-08-12: Raised to 0.55 to match paper. Be MORE selective. Only take trades the model rates >= 55% likely to profit.
ML_MIN_TRAIN_TRADES  = 20       # Min closed trades before ML gate activates
USE_LLM_GATE         = True     # Use Ollama Phi-4-mini for context-aware override
...
# =============================================================================
SYMBOL_COOLDOWN_SEC  = 60       # 1 minute — block re-entering same symbol (was 15 min, 2026-08-11)
...
# 3-symbol watchlist — 2026-08-12: SPY, QQQ, NVDA per user
# Calls only. Tested live, proven winners.
WATCHLIST = [
    "SPY",   # Tier 1 star: 80% WR, +$260 P&L
    "QQQ",   # Tier 1 star: +$632 P&L
    "NVDA",  # Tier 1: high probability setups
]  # 3 symbols — proven winners only
...
# =============================================================================
FORCED_TURBULENT_SYMBOLS = []  # 2026-08-12: cleared — TSLA caused 81 dollar loss. Use watchlist only.
...
MIN_CANDLES_FOR_SIGNAL  = 12    # 60 min minimum before any signal (was 3=15min)
#                                 # Reason: need real MACD/RSI setup, not noise

# =============================================================================
# SIGNAL QUALITY FILTERS (rejects bad signals before they become trades)
# =============================================================================
MIN_SCORE_THRESHOLD     = 50     # score must be ≥|50| to be considered (was effectively 30)
...
MODE = "live"   # FIX 2026-08-10: live config — runs only when live service started
ORDER_TYPE              = "limit"   # or "market"
LIMIT_PRICE_OFFSET      = 0.01      # $0.01 above bid for buy, below for sell
